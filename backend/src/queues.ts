...
async function handleProcessCampaign(job: Job<ProcessCampaignData>): Promise<void> {
  try {
    const { id, delay = 0 } = job.data;
    ...
    const campaignQueueOptions = { removeOnComplete: true };

    for (let i = 0; i < contacts.length; i++) {
      const contact = contacts[i];
      const variables = settings.variables;
      const delayMs = calculateDelay(i, delay, settings);

      campaignQueue.add(
        "PrepareContact",
        { contactId: contact.id, campaignId: campaign.id, variables, delay: delayMs },
        campaignQueueOptions
      );

      logger.info(`Registro enviado para a fila de disparo: Campanha=${campaign.id};Contato=${contact.name};delay=${delayMs}`);
    }
...
function calculateDelay(index: number, baseDelay: number, settings: CampaignSettings): number {
  const { messageInterval, longerIntervalAfter, greaterInterval } = settings;
  const delayMs = baseDelay + randomValue(0, messageInterval) * 1000;

  return index % longerIntervalAfter === longerIntervalAfter - 1
    ? delayMs + greaterInterval * 1000
    : delayMs;
}
